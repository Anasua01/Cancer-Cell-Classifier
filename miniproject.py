...
#After getting only numeric and non-null values, we can convert the type to int
    ...
    st.write("F1-score of our model: {}".format(sklearn.metrics.f1_score(test_y, y_hat, average="weighted")))
    st.write("Jaccard index: {}".format(sklearn.metrics.jaccard_score(test_y, y_hat, pos_label=2)))
    st.write("Shape of the dataset",data.shape)
